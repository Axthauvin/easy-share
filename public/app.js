...
function setupDragAndDropOnBubble(bubbleElement, targetId) {
  ['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => {
    bubbleElement.addEventListener(eventName, (e) => {
      e.preventDefault();
      e.stopPropagation();
    }, false);
  });
  ...
  ['dragenter', 'dragover'].forEach(eventName => {
    bubbleElement.addEventListener(eventName, () => {
...
function sendDroppedFile(file, targetId) {
  ...
  reader.onload = (event) => {
    ...
    if (socket && socket.readyState === WebSocket.OPEN) {
      ...
      const bubble = document.querySelector(`.device-bubble[data-id="${targetId}"]`);
      if (bubble) {
        const label = bubble.querySelector('.device-label');
...
function showReceivedPopup(message) {
  ...
  if (message.type === 'file') {
    const fileDataUrl = `data:${message.fileType};base64,${message.fileData}`;
    ...
    if (message.fileType.startsWith('image/')) {
      receivedModalBody.innerHTML = `
        ...
        <img src="${fileDataUrl}" class="received-image-preview" alt="Image" onclick="window.open('${fileDataUrl}')" />
        <div class="received-actions">
          <a class="action-btn-small" href="${fileDataUrl}" download="${escapeHTML(message.fileName)}">${getTranslation('saveImage')}</a>
        </div>
      `;
    } else {
      receivedModalBody.innerHTML = `
        ...
        <div class="received-actions">
          <a class="action-btn-small" href="${fileDataUrl}" download="${escapeHTML(message.fileName)}">${getTranslation('download')}</a>
        </div>
      `;
    }
  } else {
